...
import ru.kashtanov.order_service.event.OrderPlacedEvent;
...
public class AnOrderService {
    ...
    public AnOrderService(AnOrderRepository orderRepository, WebClient.Builder webClientBuilder, KafkaTemplate kafkaTemplate) {
    ...
    public void makeAnOrder(AnOrderRequestDto orderRequestDto) {
        ...
        // Here we appeal to Inventory-service for checking inventory is in the stock or not
        ...
        if (resultIsInStock) {
            ...
            kafkaTemplate.send("notification_topic", new OrderPlacedEvent(anOrder.getOrderNumber()));
        } else {
